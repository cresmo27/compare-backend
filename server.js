...
(function attachDiagEndpoint(){
  try {
    const maybeAuthSoft = (typeof authSoft === "function") ? authSoft : (req,res,next)=>next();
    const maybeRateLimit = (typeof rateLimit === "function") ? rateLimit : (req,res,next)=>next();
    const maybeProGuard = (typeof proGuard === "function") ? proGuard : (req,res,next)=>next();
    ...
    app.get("/__diag/mw", maybeAuthSoft, maybeRateLimit, maybeProGuard, (req, res) => {
      ...
      res.json({
        ...
        tag: "__diag/mw",
        middlewareFlags: {
          authSoft: !!(state && (state.authSoft || state.user)),
          rateLimitBypassed: !!(state && state.rateLimitBypassed),
          proGuard: !!(state && state.proGuard),
        },
        state,
        requestMeta: {
          hasAuthHeader: !!auth,
          authPreview: auth ? redact(auth) : null,
          xUserProviders: providers || null,
          xMode: mode || null,
          method: req.method,
          path: req.path,
        },
